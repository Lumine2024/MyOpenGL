...
struct Point3D : public glm::vec4 {
	...
	constexpr explicit Point3D(const float (&point)[3]) : glm::vec4(point[0], point[1], point[2], 1.0f) {}
	constexpr explicit Point3D(const float (&point)[4]) : glm::vec4(point[0], point[1], point[2], point[3]) {}
